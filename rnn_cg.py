...
class rnn_cg(nn.Module):
    def __init__(self,config):
        super(rnn_cg, self).__init__()
        ...
        self.classifier =  nn.Sequential(
                        nn.Linear(config.forward_size*3,config.num_labels),
                        nn.Dropout(config.drop_pro)
                    )
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, \
        ...
        # attention
        attn_energy = self.attn(lstm.reshape(-1, self.rnn_dim)) # [batch_size,sentence,hidden_size] -> [batch_size*sentence,1]
        attention_mask = attention_mask.reshape(-1,1)
        attn_energy = attn_energy*attention_mask
        attn_softmax =  F.softmax(attn_energy.view(batch_size, -1), dim=1).unsqueeze(2)
        attns = self.attn(lstm,attention_mask) # [batch_size,280,1]
        feats = (lstm * attn_softmax).sum(dim=1)     # [batch_size,280,300*2] -> [batch_size,300*2]

        # text_logits = self.fc1(lstm[:, -1, :].view(batch_size, -1))
        text_logits = self.fc1(feats)
        category_logits = self.fc2(categorys)
        logits = self.classifier(torch.cat((text_logits,category_logits,text_logits*category_logits),dim=1))
        logits = logits
